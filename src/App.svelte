<script>
  ...
  async function askGpt(r) {
    var url = 'https://api.openai.com/v1/chat/completions';
    var fo = {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${keys.cgpt}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        model: `${models.cgpt}`,
        messages: [
          { role: 'system', content: r[1] || '' },
  ...
  async function askAal(r) {
    var url = 'https://api.aleph-alpha.com/complete';
    var fo = {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${keys.aal}`,
        'Content-Type': 'application/json',
  ...
  var processingf = false;
  async function readf() {
    processingf = true;
    var ab = await files[0].arrayBuffer();
    const wb = read(ab);
    var arr = utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1 });
    var pps = [];
    for (var i = 1; i < arr.length; i++) {
      if (arr[i].length > 0) {
        if (keys.cgpt.length > 0) {
          pps.push(askGpt(arr[i]));
        }
        if (keys.aal.length > 0) {
          pps.push(askAal(arr[i]));
        }
      } else {
        break;
      }
    }
    var ra = await Promise.all(pps);
    var ras = [];
    for (var i = 0; i < ra.length; i++) {
      ras.push(ra[i].json());
    }
    var resp = await Promise.all(ras);
    arr[0][2] = 'ChatGPT';
    arr[0][3] = 'Luminous';
    for (var i = 0; i < resp.length; i++) {
      arr[i + 1][2] = resp[i].choices[0].message.content;
    }
    var ws = utils.aoa_to_sheet(arr);
    const workbook = utils.book_new();
    utils.book_append_sheet(workbook, ws, wb.SheetNames[0]);
    processingf = false;
...
<div class="inputs">
  <div class="llmcard">
    ...
    <div class="cardinputs">
      <label>API Key</label>
      <input type="text" bind:value={keys.cgpt} />
    </div>
    <div class="cardinputs">
      <label>Model</label>
      <input type="text" bind:value={models.cgpt} />
    </div>
  </div>
  <!-- <div class="llmcard">
    <h3>Luminous</h3>
...
<div class="container">
  <div class="howto">
    <h4>Howto</h4>
    <ul>
      <li>Spreadsheet has 3 columns and as many rows as you want</li>
      <li>The first row is ignored</li>
      <li>Each row is run once against ChatGPT</li>
      <li>The first columns contains the prompt</li>
      <li>The second columns contains the <a href="https://help.openai.com/en/articles/7042661-chatgpt-api-transition-guide" target="blank">system instructions</a></li>
      <li>The third will be populated with the messages from ChatGPT</li>
    </ul>
  </div>
</div>
<div class="container">
  <table>
    <caption>Spreadsheet Input Example</caption>
    <thead>
      <tr>
        <th>Prompt</th>
        <th>Instructions</th>
    ...
    <tbody>
      <tr>
        <td>Rate the following text by friendliness, use the rating very, medium and unfriendly: Hello, you are such a nice guy 🤡.</td>
        <td>You rate text and always answer with the word rating plus your rating. After the rating you explain your decision.</td>
        <td />
        <!-- <td /> -->
      </tr>
      <tr>
